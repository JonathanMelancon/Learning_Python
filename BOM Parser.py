...
hardcoded_output:bool = True # True = filename.csv --- filename_parsed.csv
...
if hardcoded_input: #same as if hardcoded_input == true
    ...
    target = filedialog.askopenfilename(title=str.upper("Select source csv file"), defaultextension=".csv",filetypes=[("Comma Separated Values", ".csv")])
...
def create_data_frame()->None:
    ...
    dataframe = pandas.DataFrame(list(zip(pop_duplicates()[0], pop_duplicates()[1], pop_duplicates()[2], web)), columns=["QTY", "PART NUMBER", "PART NAME", "WEB"])
    ...
    if hardcoded_output:
        ...
        try:
            ...
            elif overwrite == "N" or "n":
                input("File was not overwritten. Press enter to quit")


    if not hardcoded_output:

        save_csv = input("Would you like to save date to csv file? Y/N")
        ...
        if save_csv == "Y" or "y":
            ...
            root.attributes('-alpha', 0.0) # Hide the window
            root.attributes('-topmost', True) # always have it on top
            target = filedialog.asksaveasfilename(title="select destination", defaultextension=".csv",confirmoverwrite=True,filetypes=[("Comma Separated Values", ".csv")])
            root.destroy() # Destroy window when file dialog is finished
            dataframe.to_csv(target)

            print(dataframe)

        elif save_csv == "N" or "n":

            print(dataframe)

            # else:
            #     print("An error occured, please enter Y or N")
            #     SaveToCsv()
